...
#     kf = StratifiedKFold(n_splits=7, shuffle=True, random_state=seed) # No.6の異常データの数:7
#     for _, val_index in kf.split(data, label):
#         idx_list = []
#         for _, j in enumerate(val_index):
#             idx_list.append(j)
#         idx_list = np.array(idx_list)  # 7等分して、1セットずつ取得
...
#     mylist = np.array(range(len(one_set_label))) # 4:3に分けるためのインデックス取得用
#     all = itertools.combinations(mylist, 4)
#     for x in all: # 4:3のインデックス
#         x = np.array(x)
